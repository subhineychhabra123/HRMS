...
(function( $ ) {
	...
	var $itemschildren = $('.nav-sub-parent');
	...
	$items.on('click', function() {
		...
		if ( prev.get( 0 ) !== next.get( 0 ) ) {
			collapse( prev );
			expand(next);
			expand($('.nav-sub-parent'));
		} else {
			collapse( prev );
		}
	});

	$itemschildren.on('click', function (ev) {
	    if ($(this).find('.nav-sub-children').is(":visible")) {
	        $(this).find('.nav-sub-children').slideUp( 'fast');
	        $(this).removeClass('nav-expanded');
	    } else {
	        $(this).addClass('nav-expanded');
	        $(this).children('.nav-sub-children').slideDown('fast');
	    }
	    ev.stopPropagation();
	});

	$items.find('.nav-children a').on('click', function (ev) {
	    if (!$(this).parent("li").hasClass('nav-sub-parent')) {
	        ev.stopPropagation();
	    }
	});
